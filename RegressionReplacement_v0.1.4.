...
try:
    #Get a dataset
    Data = pd.read_csv("https://raw.githubusercontent.com/CharnviLopez/RegressionOptimization/main/XYregData.csv?token=GHSAT0AAAAAACIS7AIDVPA5SYSWL7B5NBXMZKJTSOQ")
    ...
    RegressionGPModel.addConstrs(z_1[i] == z[i]*z[i] for i in range(len(X)))
    
    
    

    RegressionGPModel.setObjective(quicksum(z_1[i] for i in range(len(X))), GRB.MINIMIZE)
